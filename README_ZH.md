# Macast

> **⚠️ 注意：此仓库是原项目 [xfangfang/Macast](https://github.com/xfangfang/Macast) 的 Fork。**
> 
> 由于原项目已长期未更新，无法在最新的 macOS 系统上正常运行，故在此 Fork 基础上做了以下改动以继续使用：
> - 升级所有 Python 依赖到最新版本
> - 适配 **macOS 26 Tahoe (Apple Silicon M 芯片)** 原生运行
> - 使用 `netifaces-plus` 替代已停止维护的 `netifaces`
> - 修复 py2app 打包后 .app 播放视频后无响应的问题
> - 升级 CI/CD (GitHub Actions) 到 Node.js 24，自动构建并发布 DMG
> - 最低系统要求：macOS 15.0+
>
> 原始项目的 README 见下方。向原作者 [xfangfang](https://github.com/xfangfang) 致以感谢。
